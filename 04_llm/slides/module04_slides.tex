...
\begin{frame}{Tokenization: Teks Menjadi Angka}
  \framesubtitle{Ingat dari Module 03 --- kali ini kita lihat cara kerjanya di dalam LLM}
  {\color{nvgray}\small Ingat dari Module 03: LLM memakai \textbf{subword tokenization} --- kamu sudah melihat ``mempelajari'' dipecah jadi potongan subword: \texttt{['mem','pel','aj','ari']}.}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \footnotesize
...
\begin{frame}{Ringkasan Modul 04}
  ...
  \begin{tabular}{@{}p{0.05\linewidth}p{0.20\linewidth}p{0.40\linewidth}p{0.28\linewidth}@{}}
    ...
    2 & Pipeline vs manual & \texttt{pipeline()} vs AutoModel (TinyLlama-1.1B-Chat) & 2 cara load \& jalankan model \\
...
\begin{frame}[fragile]{Setup, Tools \& Lanjut ke Module 05}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \footnotesize
        \item Model kecil: \texttt{opt-350m}, \texttt{bert-tiny}, \texttt{TinyLlama-1.1B}
