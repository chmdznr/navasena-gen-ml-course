...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{Tag \texttt{<think></think>} kosong masih bisa muncul} (thinking-token Qwen3). Upgrade ke $\geq$ 0.30.7 memperbaiki token think yang \emph{rusak/setengah jadi}, \textbf{tetapi} karena Modelfile kita memakai TEMPLATE ChatML sendiri, Ollama tidak mengenali model ini sebagai thinking-model $\to$ blok \texttt{<think></think>} kosong ikut tercetak sebagai teks (bahkan dengan \texttt{--think=false}). \textbf{Tidak merusak JSON}; bersihkan di sisi klien (Bagian D).
...
out = re.sub(r"<think>.*?</think>", "", out, flags=re.DOTALL).strip()  # buang blok think Qwen3 (kebal versi)
print(out)
\end{lstlisting}
...
\rowcolor{nvbg}Output \texttt{<think></think>\{json\}} & TEMPLATE ChatML custom $\to$ Ollama tidak parse thinking-token Qwen3 (flag \texttt{--think} pun no-op) & blok kosong \textbf{tidak merusak JSON}; strip regex di sisi klien (Bagian D) \\
