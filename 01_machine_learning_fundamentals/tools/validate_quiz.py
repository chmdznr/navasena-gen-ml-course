#!/usr/bin/env python3
"""Gate for the reworked Module 01 ML quiz. Parses the inline QUIZ JSON and checks invariants.
Mirrors 04_llm/tools/validate_quiz.py."""
import re, json, sys, pathlib

ROOT = pathlib.Path(__file__).resolve().parent.parent
HTML = (ROOT / "ml-fundamentals-quiz.html").read_text(encoding="utf-8")
errs = []
...
for kw in ["leakage", "baseline", "cross-validation", "overfitting", "recall", "r²", "threshold", "pengembalian", "silhouette", "overhead", "early stopping", "seasonal", "scaling", "support vector"]:
    if kw not in blob:
...
print(f"QUIZ OK: {n} pure-concept questions, header matches, M01 concept coverage.")
