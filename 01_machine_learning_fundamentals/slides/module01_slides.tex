...
\begin{frame}{Benchmark: syarat pengukurannya disebut}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{center}
        \includegraphics[width=\textwidth, height=4.3cm, keepaspectratio]{figures/gpu_speedup.pdf}
      \end{center}
      \vspace{0.1em}
      {\scriptsize\color{nvgray}Panel kiri akan diisi dari hasil Colab T4 di nb10.}
    \end{column}
    \begin{column}{0.47\textwidth}
      \vspace{0.1em}
      \begin{itemize}\setlength\itemsep{0.25em}
        \item \textbf{Tugas}: Random Forest 200 pohon, KNN, K-Means, dan XGBoost.
        \item \textbf{Data}: Covertype, 581.012 baris (KNN: subset 100.000 baris train).
        \item \textbf{Yang diukur}: waktu \texttt{fit()}, setelah warm-up.
        \item \textbf{Perangkat}: satu GPU Tesla T4 di Google Colab, dibandingkan CPU pada
              runtime yang sama.
      \end{itemize}
      \vspace{0.25em}
      {\footnotesize\color{nvgray}Selama angkanya belum diukur di perangkat itu, tidak ada rasio
      percepatan yang bisa disebut di sini.}
    \end{column}
  \end{columns}
\end{frame}
