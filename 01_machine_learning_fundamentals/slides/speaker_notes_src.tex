...
\subsection*{\framebox{Slide 13/88}\quad Scaling: menyamakan rentang antar feature}
...
\textbf{Ucapkan:} "Cara kedua menanyakan hal yang lebih tepat. Silhouette mengukur, untuk setiap baris, seberapa dekat baris itu ke anggota kelompoknya sendiri dibanding ke kelompok tetangga terdekat. Rentangnya minus satu sampai satu; makin tinggi makin jelas pemisahannya. Di data ini skornya paling tinggi di K sama dengan dua, sekitar nol koma dua delapan, lalu turun untuk K yang lebih besar. Yang perlu diingat, angka ini satu bahan pertimbangan, bukan perintah. Kalau kebutuhan bisnis meminta segmen yang lebih halus, K tiga sampai empat masih wajar; skornya turun sedikit, dan penurunan itu bagian dari keputusan yang kita ambil sadar. Yang tidak boleh: memilih K dari skor lalu menceritakannya sebagai jumlah kelompok yang sebenarnya ada."\\[2pt]
\textbf{Baca rumus:}\\[2pt]
\begin{itemize}\setlength\itemsep{0.2em}
...
\textbf{Ucapkan:} "Metode ketiga berangkat dari kepadatan, bukan dari jarak ke pusat. Aturannya: baris yang punya paling sedikit \texttt{min\_samples} tetangga di dalam radius \texttt{eps}, lingkaran oranye itu, menjadi inti kelompok, dan tetangganya ikut bergabung. Kelompok tumbuh dari inti ke inti. Dua sifat yang membedakannya dari k-means. Pertama, baris yang tidak masuk kelompok mana pun ditandai sebagai outlier, tanda silang merah itu, bukan dipaksa masuk ke kelompok terdekat. Kedua, jumlah kelompoknya tidak ditentukan di awal dan bentuknya bebas, jadi kelompok memanjang pun bisa ditemukan. Harganya: dua parameter yang harus dipilih. \texttt{eps} diambil dari k-distance plot, yaitu urutkan jarak ke tetangga ke-k lalu ambil nilai di tempat kurvanya mulai menanjak."\\[2pt]
...
\textbf{Ucapkan:} "Slide ini sengaja tidak menyebut angka percepatan, dan itu bagian dari pesannya. Angka di panel kiri diisi dari hasil pengukuran nb10 di Colab dengan Tesla T4, jadi selama sesi ini belum menjalankan notebooknya, tidak ada rasio yang bisa saya sebutkan. Yang bisa disebut sekarang syarat pengukurannya, dan justru itu yang perlu kamu tiru. Tugasnya: Random Forest dua ratus pohon, KNN, K-Means, dan XGBoost. Datanya Covertype, lima ratus delapan puluh satu ribu baris. Yang diukur: waktu \texttt{fit()} saja, dan diambil setelah warm-up. Perangkatnya satu GPU Tesla T4 di Google Colab, dibandingkan CPU pada runtime yang sama. Nanti setelah nb10 jalan, kita isi angkanya bersama dan lihat mana yang tidak sesuai dugaan."\\[2pt]
