...
\hyphenation{train vari-ance scal-ing de-ci-sion boost-ing gra-di-ent
  ...
  pool-ing con-vo-lu-tion em-bed-ding gene-ra-tor dis-cri-mi-na-tor}
\usepackage{amsmath}
\usepackage{amssymb}
...
\subsection*{\framebox{Slide 1/74}\quad Judul: Deep Learning Fundamentals}
...
\subsection*{\framebox{Slide 2/74}\quad Peta hari 2}
...
\subsection*{\framebox{Slide 3/74}\quad Dari hari 1 ke hari 2}
...
\subsection*{\framebox{Slide 4/74}\quad Act 1: Bagaimana neural network belajar?}
...
\subsection*{\framebox{Slide 5/74}\quad Intuisi: Satu neuron: jumlah berbobot, lalu diputuskan}
...
\subsection*{\framebox{Slide 6/74}\quad Dari satu neuron ke satu arsitektur}
...
\subsection*{\framebox{Slide 7/74}\quad Yang berubah saat model belajar}
...
\subsection*{\framebox{Slide 8/74}\quad Loss: satu angka yang mengukur seberapa salah}
...
\subsection*{\framebox{Slide 9/74}\quad Intuisi: Gradient descent: menuruni bukit dalam kabut}
...
\subsection*{\framebox{Slide 10/74}\quad Learning rate: ukuran langkah dan gejalanya}
\addcontentsline{toc}{subsection}{Slide 10: Learning rate}
\textbf{Ucapkan:} "Ukuran langkah tadi punya nama: learning rate. Mulai dari yang terlalu kecil. Pada gambar sebelumnya, learning rate yang kecil, 0,05, membuat loss turun perlahan: dua belas langkah dan belum sampai ke dasar. Gejalanya di notebook: batas epoch tercapai, tetapi loss masih turun perlahan. Sekarang yang terlalu besar, 1,05. Langkahnya melewati dasar lalu mendarat lebih tinggi dari sebelumnya, dan tiap langkah berikutnya makin jauh. Gejalanya loss naik-turun tanpa arah, atau muncul NaN. Di antara keduanya, 0,25 sampai ke dasar lewat beberapa langkah bertahap: langkah-langkah awal masih lebar, lalu mendekati dasar pada ilustrasi ini jarak antarlangkah mengecil. Satu jebakan yang perlu disebut: Adam memakai learning rate 0,001 sebagai default, dan itu titik awal yang wajar, bukan angka yang cocok untuk semua data. Kalau loss tidak berubah atau menjadi NaN, periksa nilai loss, gradient, serta skala data terlebih dahulu. Learning rate perlu diperiksa, tetapi belum tentu menjadi satu-satunya penyebab. Menambah layer belum menjawab masalah tersebut."\\[2pt]
\textbf{Baca rumus:} aturan yang mendasari ketiga keadaan ini, walau tidak tercetak di slide, adalah $w \leftarrow w - \eta \cdot \partial L/\partial w$. Bacanya: ``w baru sama dengan w lama dikurangi eta kali turunan loss terhadap w''. Sebut sekali bahwa ``eta'' itulah learning rate, dan tanda minus di depan yang membuat langkahnya mengarah turun, bukan naik.\\[2pt]

\subsection*{\framebox{Slide 11/74}\quad Intuisi: Backprop: gradient dihitung dari output ke layer awal}
\addcontentsline{toc}{subsection}{Slide 11: Intuisi: Backprop}
\textbf{Ucapkan:} "Satu pertanyaan yang belum terjawab: turunan loss terhadap seratus ribu bobot itu dihitung bagaimana. Yang ingin kita tahu sederhana: bagaimana loss berubah jika sebuah bobot diubah sedikit. Ikuti dua panah di gambar. Panah hijau adalah arah maju: data masuk, tiap layer menghitung keluarannya, sampai muncul prediksi dan satu angka loss. Panah oranye arah mundur. Backprop menghitung gradient loss terhadap tiap bobot dengan menelusuri perhitungan itu dari output ke layer sebelumnya. Hasilnya bukan potongan error yang dibagi-bagi, melainkan turunan: seberapa peka loss terhadap tiap bobot. Angka itulah yang dipakai optimizer untuk memperbarui bobot. Jadi bagi tugasnya menjadi dua: backprop menghitung gradient, optimizer memperbarui bobot. Di Keras keduanya terjadi di dalam \texttt{fit()}."\\[2pt]

\subsection*{\framebox{Slide 12/74}\quad Epoch, batch, dan satu langkah perbaikan}
\addcontentsline{toc}{subsection}{Slide 12: Epoch dan batch}
\textbf{Ucapkan:} "Tiga istilah yang akan terus muncul di log training, dan sering tertukar. Batch: sekelompok contoh yang dihitung bersama, di nb01 isinya 32 gambar. Satu batch memberi satu langkah perbaikan bobot, jadi tiap kotak di gambar punya panah ke bawah bertuliskan bobot diperbarui. Epoch: seluruh data train dilewati sekali. Di nb01 data trainnya 55.000 gambar; dibagi 32, jadi satu epoch berisi 1.719 langkah perbaikan. Jadi setelah beberapa epoch saja, bobotnya sudah diperbarui belasan ribu kali. Ukuran batch punya kompromi: batch besar membuat arah langkahnya lebih tenang karena dihitung dari lebih banyak contoh, tetapi butuh memori lebih besar dan langkahnya lebih sedikit per epoch. Angka di slide dari nb01: 55.000 train, 5.000 validation, 10.000 test."\\[2pt]

\subsection*{\framebox{Slide 13/74}\quad Kurva train dan validation dibaca berdampingan}
\addcontentsline{toc}{subsection}{Slide 13: Kurva train dan validation}
\textbf{Ucapkan:} "Kurva ini dari run nyata model Dense(128) pada Fashion-MNIST, delapan epoch, batch 128, \texttt{validation\_split} 0,1 — bukan model nb01 yang punya dua hidden layer, tetapi polanya sama dengan yang akan kamu lihat di nb01. Cara membacanya perlu kita sepakati sekarang karena gambar seperti ini muncul di lima notebook berikutnya. Dua panel. Panel kiri loss, panel kanan accuracy. Di tiap panel ada dua kurva: data train dan data validation. Di run ini kedua kurva loss masih turun bersama, dan akurasi validation berhenti di sekitar 0,88. Aturan bacanya: selama kedua kurva loss masih turun bersama, model masih belajar hal yang berlaku umum. Kalau loss validation mulai naik sementara loss train terus turun, saat itulah model mulai menghafal data train. Satu batas dari run ini: garis putus-putus menandai epoch dengan val\_loss terendah, dan pada run ini titik itu jatuh di epoch terakhir. Artinya delapan epoch belum cukup untuk melihat titik baliknya, jadi jangan simpulkan modelnya sudah selesai belajar."\\[2pt]

\subsection*{\framebox{Slide 14/74}\quad Overfitting: melebarnya jarak train dan validation}
\addcontentsline{toc}{subsection}{Slide 14: Overfitting, dropout, early stopping}
\textbf{Ucapkan:} "Supaya titik baliknya terlihat, gambar ini dibuat sengaja: model yang sama dilatih pada 6.000 sampel train saja, jadi ada lebih sedikit contoh untuk dihafal ulang. Lihat selisih kedua panel. Tanpa dropout, jarak antara loss validation dan loss train di akhir melebar sampai 0,60; itu angka yang tercetak di judul panel kiri. Dengan \texttt{Dropout(0.5)}, pada percobaan ini jaraknya 0,18. Jarak yang melebar itulah gejala overfitting, tetapi jarak bukan satu-satunya ukuran: nilai loss validationnya juga perlu dilihat. Dua cara menahannya. Saat training, dropout menolkan sebagian aktivasi secara acak, jadi model tidak bisa bergantung pada satu jalur saja. Early stopping menghentikan training jika metrik yang dipantau tidak membaik sesuai batas patience; di Keras, pemulihan bobot terbaik bukan default, perlu \texttt{restore\_best\_weights=True}. Satu hal yang perlu disebut: dropout hanya aktif saat training. Loss validation bisa lebih rendah daripada loss train, dan salah satu penyebabnya dropout tidak aktif saat evaluasi sehingga seluruh neuron ikut bekerja."\\[2pt]

\subsection*{\framebox{Slide 15/74}\quad Normalisasi input: dari 0--255 ke 0,0--1,0}
...
\subsection*{\framebox{Slide 16/74}\quad Evaluasi multi-kelas: akurasi saja belum cukup}
...
\subsection*{\framebox{Slide 17/74}\quad Ringkasan Act 1}
\addcontentsline{toc}{subsection}{Slide 17: Ringkasan Act 1}
\textbf{Ucapkan:} "Lima hal dari blok ini, dan kelimanya dipakai sampai malam. Satu, neuron menghitung jumlah berbobot dari masukannya, menambah bias, lalu melewatkannya ke aktivasi; layer adalah kumpulan neuron seperti itu. Dua, loss meringkas kesalahan menjadi satu angka, backprop menghitung gradient loss terhadap tiap bobot, dan optimizer memakai gradient itu untuk memperbarui bobotnya. Tiga, learning rate menentukan ukuran langkah: terlalu kecil membuat training lambat, terlalu besar membuatnya meledak, dan itu hal pertama yang diperiksa saat loss tidak turun. Empat, kurva train dan validation dibaca bersama; jarak yang melebar tanda overfitting, dan dropout serta early stopping menahannya. Lima, untuk klasifikasi banyak kelas, confusion matrix menunjukkan kesalahan yang tidak terlihat dari angka akurasi."\\[2pt]

\section{Act 2 — Kenapa butuh aktivasi?}

\subsection*{\framebox{Slide 18/74}\quad Act 2: Kenapa butuh aktivasi?}
...
\subsection*{\framebox{Slide 19/74}\quad Tanpa aktivasi, tumpukan layer tetap satu garis}
...
\subsection*{\framebox{Slide 20/74}\quad Empat aktivasi dan turunannya}
\addcontentsline{toc}{subsection}{Slide 20: Empat aktivasi dan turunannya}
\textbf{Ucapkan:} "Empat aktivasi inti di gambar ini; nb02 juga menampilkan ELU dan Swish, dan membandingkan lima di antaranya di data validation. Gambar ini punya dua baris yang perlu dibedakan. Baris atas fungsinya: bentuk keluaran neuron untuk tiap nilai masukan. Baris bawah turunannya, dan turunan inilah yang ikut masuk ke perhitungan gradient saat backprop menelusuri jaringan ke belakang. Jadi baris bawah yang menentukan bobotnya bergerak atau tidak. Sekarang lihat daerah merah di baris bawah. Di sana turunannya mendekati nol, artinya kalau masukan satu neuron jatuh di daerah itu, bobotnya hampir tidak berubah walau modelnya masih salah. Sigmoid dan tanh punya daerah merah di kedua ujungnya, dan itu yang jadi masalah di slide berikutnya. Bandingkan dengan ReLU: turunannya rata satu di seluruh sisi positif, tanpa daerah merah di sana. Kolom paling kanan sengaja diberi warna lain: sisi negatif Leaky ReLU turunannya 0,1 — kecil, tetapi bukan nol, jadi bobotnya masih bergerak."\\[2pt]

\subsection*{\framebox{Slide 21/74}\quad Intuisi: Vanishing gradient: pesan yang habis di jalan}
\addcontentsline{toc}{subsection}{Slide 21: Intuisi: Vanishing gradient}
\textbf{Ucapkan:} "Bayangkan sinyal yang melemah setiap kali melewati satu tahap, seperti bisik berantai. Bedanya, yang menyusut di sini bukan isi pesannya, melainkan besar gradient, dan pelemahannya bisa kita hitung. Turunan sigmoid nilai terbesarnya 0,25, jadi tiap kali gradient ditelusuri melewati satu layer sigmoid, faktor dari aktivasi itu paling banyak 0,25. Ikuti angka di gambar: mulai dari satu di loss, lalu dikali 0,25 tiga kali, hasilnya sekitar 0,016. Sebut asumsinya: untuk melihat efek turunannya saja, contoh ini menganggap faktor dari bobot bernilai satu. Pada jaringan sebenarnya, faktor dari bobot juga ikut menentukan hasilnya. Jadi contoh ini menunjukkan salah satu penyebab gradient mengecil saat ditelusuri ke layer awal. Itu namanya vanishing gradient."\\[2pt]

\subsection*{\framebox{Slide 22/74}\quad ReLU: turunannya tetap 1 di sisi positif}
\addcontentsline{toc}{subsection}{Slide 22: ReLU dan Leaky ReLU}
\textbf{Ucapkan:} "Jawaban paling banyak dipakai untuk masalah tadi, dan bentuknya sesederhana dua garis di kiri. ReLU meneruskan nilai positif apa adanya dan menolkan yang negatif. Karena sisi positifnya garis lurus dengan kemiringan satu, turunannya di sana tetap satu; pada jalur yang aktif, aktivasi ini tidak memperkecil gradient melalui turunannya sendiri. Hitungannya juga murah: satu perbandingan dengan nol per neuron, tanpa fungsi eksponensial. Tetapi ada harganya. Sisi negatifnya nol rata, jadi turunannya nol juga. Jika nilai sebelum ReLU selalu negatif pada data train, neuron itu terus menghasilkan nol dan tidak menerima gradient melalui jalur aktivasi tersebut; keadaan itu disebut dead ReLU. Leaky ReLU memberi kemiringan kecil di sisi negatif, 0,1 pada gambar slide sebelumnya, sehingga masih ada gradient yang mengalir dan neuronnya bisa pulih."\\[2pt]
...
\subsection*{\framebox{Slide 23/74}\quad Softmax: skor mentah menjadi probabilitas}
...
\subsection*{\framebox{Slide 24/74}\quad Intuisi: Optimizer: cara berbeda menuruni bukit yang sama}
...
\subsection*{\framebox{Slide 27/74}\quad Act 3: Bagaimana komputer melihat gambar?}
...
\subsection*{\framebox{Slide 28/74}\quad Gambar sampai ke model sebagai tabel angka}
...
\subsection*{\framebox{Slide 29/74}\quad Intuisi: Konvolusi: satu filter kecil digeser ke seluruh gambar}
...
\subsection*{\framebox{Slide 30/74}\quad Mekanisme konvolusi: satu jumlah berbobot per posisi}
\addcontentsline{toc}{subsection}{Slide 30: Mekanisme konvolusi}
\textbf{Ucapkan:} "Sekarang mekanismenya dalam angka. Input lima kali lima di kiri. Kotak hijau menandai posisi filter saat ini, tiga kali tiga, dan filternya di tengah: sembilan bobot dan satu bias. Di posisi itu, sembilan angka input dikalikan sembilan bobot filter lalu dijumlahkan, dan hasilnya satu angka, satu kotak hijau di peta keluaran sebelah kanan. Filter digeser satu langkah, hitung lagi, satu kotak lagi. Lima kali lima dengan filter tiga kali tiga memberi keluaran tiga kali tiga. Dua hal yang menjelaskan kenapa cara ini cocok untuk gambar. Pertama, bobot filter yang sama dipakai di setiap posisi, jadi model dapat mencari pola lokal yang sama di berbagai bagian gambar. Kalau ada yang bertanya istilahnya: menggeser input menggeser respons pada feature map, dan sifat itu disebut translation equivariance --- bukan jaminan hasil akhirnya tidak berubah. Kedua, hitung parameternya. \texttt{Conv2D(32, (3,3))} pada input RGB memakai 32 kali kurung 3 kali 3 kali 3 tambah 1, sama dengan 896 parameter, termasuk bias. Satu Dense layer dari 3.072 nilai masukan ke 1.024 neuron memakai 3.146.752 parameter, juga termasuk bias. Angka ini membandingkan cara kedua layer terhubung, bukan membuktikan keduanya menghasilkan representasi yang setara."\\[2pt]
...
\subsection*{\framebox{Slide 31/74}\quad Feature map: satu peta untuk tiap filter}
...
\subsection*{\framebox{Slide 32/74}\quad Pooling: menyusutkan peta, menyimpan yang paling kuat}
...
\subsection*{\framebox{Slide 33/74}\quad Arsitektur CNN nb03 dari ujung ke ujung}
\addcontentsline{toc}{subsection}{Slide 33: Arsitektur CNN nb03}
\textbf{Ucapkan:} "Semua bagian tadi sekarang dirangkai, dan ini model nb03 apa adanya. Masuk 32 kali 32 kali 3. Blok pertama, Conv2D 32 filter lalu MaxPool, keluar 16 kali 16 kali 32. Blok kedua, Conv2D 64 lalu MaxPool, keluar 8 kali 8 kali 64. Blok ketiga, Conv2D 128 lalu MaxPool, keluar 4 kali 4 kali 128. Baca angkanya berpasangan: ukuran petanya menyusut, jumlah filternya bertambah. Lalu Flatten menggelar 4 kali 4 kali 128 menjadi 2.048 angka, masuk Dense 128 dengan Dropout 0,5, dan berakhir di Dense 10 dengan softmax. Pembagian tugasnya jelas: bagian konvolusi menyusun feature dari pixel, bagian Dense di ujung memakai feature itu untuk memilih satu dari sepuluh kelas. Bagian ujung itu yang disebut classification head, dan istilah itu dipakai lagi dua slide berikutnya."\\[2pt]

\subsection*{\framebox{Slide 34/74}\quad Augmentasi sebagai layer di dalam model}
\addcontentsline{toc}{subsection}{Slide 34: Augmentasi sebagai layer}
\textbf{Ucapkan:} "Cara menambah ragam data train tanpa mengumpulkan gambar baru. Di kiri, tiga layer augmentasi dipasang di dalam model, tepat setelah Input dan sebelum Conv2D: \texttt{RandomFlip}, \texttt{RandomRotation(0,04)}, \texttt{RandomZoom(0,1)}. Karena bentuknya layer, ketiganya ikut aturan Keras: aktif saat \texttt{fit()}, mati saat evaluasi. Jadi gambar yang sama muncul dibalik, diputar sedikit, atau di-zoom ulang di setiap epoch, dan model tidak melihat versi yang persis sama dua kali. Butir kedua yang perlu ditimbang tiap kali: ragamnya dipilih sesuai datanya. Kucing yang dibalik tetap kucing, tetapi angka atau tulisan yang dibalik bisa berubah arti. Satu hal yang sering membingungkan: karena gambar train diacak sementara data validation tidak, akurasi train bisa terlihat lebih rendah daripada akurasi validation di awal training. Itu efek augmentasi, bukan tanda ada yang salah."\\[2pt]

\subsection*{\framebox{Slide 35/74}\quad Intuisi: Transfer learning: meminjam mata yang sudah terlatih}
\addcontentsline{toc}{subsection}{Slide 35: Intuisi: Transfer learning}
\textbf{Ucapkan:} "Kalau kamu punya dua ribu gambar dan butuh model gambar yang bagus, melatih dari nol bukan satu-satunya jalan. Bayangkan meminjam mata orang yang sudah bertahun-tahun melihat gambar: dia sudah tahu apa itu tepi, apa itu tekstur, apa itu roda. Yang perlu diajarkan hanya nama-nama kelas di pekerjaanmu. Mekanismenya persis susunan kotak di gambar. ResNet50 sudah dilatih di ImageNet, 14 juta gambar dan 1.000 kelas. Layer awalnya mempelajari hal yang umum untuk gambar apa pun: tepi dan warna, lalu tekstur dan sudut, lalu bagian objek seperti roda atau mata. Bagian di dalam kotak putus-putus itu disebut backbone: tugasnya mengubah gambar menjadi representasi. Yang kita tambahkan hanya kotak hijau di kanan, head baru untuk sepuluh kelas CIFAR-10, dan hanya bagian itu yang dilatih dengan data kita. Dua istilah itu, backbone dan head, dipakai terus mulai dari sini."\\[2pt]

\subsection*{\framebox{Slide 36/74}\quad Bobot backbone tetap, head dilatih}
\addcontentsline{toc}{subsection}{Slide 36: Bobot backbone tetap, head dilatih}
\textbf{Ucapkan:} "Di sini kita memakai ResNet50 yang sudah dilatih pada ImageNet. Bagian tanpa classifier bawaannya menjadi backbone. Tugasnya mengolah gambar menjadi representasi yang bisa dipakai untuk klasifikasi.

Kita tambahkan head untuk sepuluh kelas pada latihan ini. Lihat urutannya: feature map diringkas menjadi satu vektor dengan \texttt{GlobalAveragePooling2D}. Setelah itu ada \texttt{Dropout(0.5)}, lalu Dense dengan 128 neuron, lalu output untuk sepuluh kelas.

Pada tahap pertama, \texttt{trainable} pada backbone kita set ke \texttt{False}. Artinya, bobot backbone tidak diperbarui saat training. Yang dilatih adalah bobot pada head. Jadi, gambar tetap diproses oleh backbone; membekukan bobot tidak menghilangkan perhitungan tersebut. Karena itu, waktu per epoch tetap perlu diukur.

Setelah head dilatih, kita bisa mempertimbangkan fine-tuning. Pada alur ini, beberapa layer backbone dibuka untuk dilatih kembali dengan learning rate yang lebih kecil. Model kemudian dikompilasi ulang. Tujuannya menyesuaikan representasi dengan tugas baru sambil membatasi perubahan pada bobot pralatih. Keputusan untuk melanjutkan tetap melihat hasil pada data validation."\\[2pt]

\subsection*{\framebox{Slide 37/74}\quad Kapan transfer learning menolong, kapan tidak}
\addcontentsline{toc}{subsection}{Slide 37: Kapan transfer learning menolong}
\textbf{Ucapkan:} "Halaman yang paling sering dilewati padahal paling menghemat waktu. Kolom kiri, keadaan yang cocok: datanya sedikit, ribuan gambar bukan jutaan; resolusinya mendekati 224 kali 224, ukuran yang dipakai backbonenya belajar; dan objeknya sejenis foto sehari-hari yang ada di ImageNet. Kolom kanan, keadaan yang kurang cocok: gambarnya kecil, 32 kali 32 seperti CIFAR-10; domainnya jauh dari foto biasa, misalnya citra medis atau radar; atau datanya justru besar dan cukup untuk melatih CNN dari nol. Sekarang hasilnya di nb03. ResNet50 yang dibekukan tidak otomatis mengalahkan CNN kecil yang dilatih dari nol pada CIFAR-10, karena gambar 32 kali 32 harus diperbesar dulu sebelum masuk backbone yang belajar di 224 kali 224, dan detail yang hilang tidak kembali. Jadi jangan disimpulkan dari nama modelnya; bandingkan keduanya di data validation."\\[2pt]

\subsection*{\framebox{Slide 38/74}\quad Ringkasan Act 3}
\addcontentsline{toc}{subsection}{Slide 38: Ringkasan Act 3}
\textbf{Ucapkan:} "Lima hal dari blok gambar. Satu, gambar masuk ke model sebagai tabel angka, dan konvolusi memanfaatkan kedekatan antar pixel yang diabaikan Dense layer. Dua, satu filter kecil digeser ke seluruh gambar dan menghasilkan satu feature map; bobot filter yang sama dipakai di setiap posisi, jadi parameternya jauh lebih sedikit. Tiga, pooling menyusutkan peta dengan menyimpan nilai terkuat, tanpa menambah parameter. Empat, blok Conv lalu Pool ditumpuk sampai petanya kecil, lalu Flatten dan Dense mengambil keputusan kelas. Lima, augmentasi sebagai layer hanya aktif saat training, dan transfer learning memakai backbone terlatih lalu melatih headnya, dengan keunggulan yang bergantung pada resolusi serta kedekatan domain."\\[2pt]

\section{Act 4 — Bagaimana model membaca urutan?}

\subsection*{\framebox{Slide 39/74}\quad Act 4: Bagaimana model membaca urutan?}
...
\subsection*{\framebox{Slide 40/74}\quad Data berurutan: posisinya ikut membawa arti}
...
\subsection*{\framebox{Slide 41/74}\quad Intuisi: RNN: satu catatan ringkas yang dibawa terus}
...
\subsection*{\framebox{Slide 42/74}\quad Mekanisme RNN: bobot yang sama dipakai di tiap langkah}
...
\subsection*{\framebox{Slide 50/74}\quad Act 5: Kapan GPU benar-benar diperlukan?}
\addcontentsline{toc}{subsection}{Slide 50: Act 5 (pembuka)}
\textbf{Ucapkan:} "Blok kelima, dan berhenti soal arsitektur. Empat notebook tadi semuanya dijalankan di GPU, dan pertanyaannya kenapa, sampai batas mana, dan apa yang dilakukan setelah training selesai. Judul kecil di bawah menyebut jalurnya: dari perkalian matriks sampai TensorRT."\\[2pt]

\subsection*{\framebox{Slide 51/74}\quad Training berisi banyak perkalian matriks besar}
\addcontentsline{toc}{subsection}{Slide 51: Training dan perkalian matriks}
\textbf{Ucapkan:} "Sebelum bicara perangkat, kita lihat bentuk pekerjaannya. Satu batch masukan berukuran 64 kali 784: 64 gambar, tiap gambar 784 angka. Bobot satu layer Dense berukuran 784 kali 128. Hasil perkaliannya grid di kanan, 64 kali 128. Sekarang kalimat di kanan gambar, dan ini inti seluruh act ini: tiap sel keluaran dihitung dari barisnya sendiri dan kolomnya sendiri, jadi urutan pengerjaannya tidak penting. Empat sel hijau di grid itu bisa dihitung bersamaan tanpa saling menunggu. Dua butir berikutnya soal jumlahnya. Satu langkah training menghitung prediksi maju, lalu menghitung gradient untuk tiap bobot mundur, dan keduanya berisi perkalian matriks seperti ini. Benchmark nb05 melatih CNN pada 10.000 gambar CIFAR-10 dengan batch 64 selama beberapa epoch, jadi perkalian seperti ini terjadi ribuan kali."\\[2pt]

\subsection*{\framebox{Slide 52/74}\quad Intuisi: CPU sedikit tapi kuat, GPU banyak tapi sederhana}
...
\subsection*{\framebox{Slide 53/74}\quad Kapan GPU tidak memberi keuntungan}
\addcontentsline{toc}{subsection}{Slide 53: Kapan GPU tidak untung}
\textbf{Ucapkan:} "Ada beberapa kondisi ketika CPU bisa menyelesaikan pekerjaan lebih cepat. Modelnya kecil: untuk beberapa layer Dense di atas data tabular, overhead penggunaan GPU dapat lebih besar daripada penghematan waktu komputasinya. Batchnya kecil: batch kecil dapat membuat kemampuan GPU kurang terpakai, terutama jika perhitungan per sampelnya ringan. Datanya perlu dipindahkan: jika data masih berada di memori CPU, pemindahannya ke GPU ikut menambah waktu. Dan pemanggilan pertama: di sana bisa termasuk inisialisasi runtime dan tracing graph, sehingga waktunya lebih lama daripada pemanggilan berikutnya. Itulah alasan kita melakukan warm-up sebelum mengukur pemanggilan berikutnya. Di nb05, jalur CPU dan GPU sama-sama dijalankan sebelum waktu training dan inferensi diukur. Sebut batasnya: pengukuran ini menunjukkan waktu pemanggilan pada alur notebook setelah warm-up, dan waktu tersebut masih dapat mencakup pemindahan data serta overhead lainnya. Kalau yang ingin kita ukur justru pemanggilan pertama, laporkan waktunya secara terpisah. Kalau prosedurnya sudah diperiksa dan CPU tetap lebih cepat, laporkan hasil itu."\\[2pt]

\subsection*{\framebox{Slide 54/74}\quad Mixed precision: hitung di float16, keluaran di float32}
\addcontentsline{toc}{subsection}{Slide 54: Mixed precision}
\textbf{Ucapkan:} "Cara paling murah mempercepat training di GPU modern: tiga baris kode di atas. Baris pertama impor, baris kedua menyalakan policy mixed float enam belas untuk seluruh model, baris ketiga mengecualikan output layer. Alasannya di butir pertama dan kedua. Float enam belas memakai 16 bit per angka, float tiga dua memakai 32 bit, jadi angkanya lebih hemat memori dan lebih cepat dipindahkan, dan GPU NVIDIA modern punya Tensor Core yang memang dibuat untuk operasi float enam belas. Tetapi ada bagian yang rawan kalau jangkauan angkanya dipersempit: keluaran softmax dan penjumlahan gradient. Bagian itu dibiarkan di float tiga dua, dan karena itu output layernya diberi \texttt{dtype} sama dengan float tiga dua. Batasnya perlu disebut. Keuntungannya terasa pada model dan batch yang cukup besar, dan besarnya diukur di nb05, bukan diperkirakan. Catatan kecil di bawah: di nb05 policynya dikembalikan ke float tiga dua setelah percobaan supaya sel-sel berikutnya tidak ikut terpengaruh."\\[2pt]

\subsection*{\framebox{Slide 55/74}\quad Dari training ke inferensi: ONNX lalu TensorRT}
\addcontentsline{toc}{subsection}{Slide 55: ONNX dan TensorRT}
\textbf{Ucapkan:} "Sampai sini semua soal training. Sekarang modelnya sudah jadi dan tinggal dipakai, dan pekerjaan itu punya alat sendiri. Ikuti rantai di gambar: model Keras hasil training diekspor menjadi SavedModel, dikonversi dengan \texttt{tf2onnx} menjadi ONNX, lalu dibangun dengan Polygraphy menjadi engine TensorRT FP16, dan engine itulah yang dipakai melayani inferensi. Kenapa lewat ONNX? Karena ONNX format perantara: sekali model ada di sana, banyak mesin inferensi bisa membacanya tanpa peduli framework asalnya. Sekarang syarat versinya, dan bagian ini penting supaya notebooknya tidak gagal di kelas. TF-TRT, integrasi bawaan TensorFlow, sudah dihapus sejak TensorFlow 2.18; versi 2.17 yang terakhir memuatnya, dan Colab sekarang memakai 2.18 ke atas. Karena itu jalurnya lewat ONNX. Lalu dua pin di baris \texttt{pip} itu: \texttt{tensorrt} dipin ke seri sepuluh titik x, karena resep nb05 diverifikasi pada seri itu; dan \texttt{tf2onnx} minimal 1.17 supaya cocok dengan NumPy 2."\\[2pt]

\subsection*{\framebox{Slide 56/74}\quad Benchmark pada konfigurasi kelas}
\addcontentsline{toc}{subsection}{Slide 56: Benchmark GPU}
\textbf{Ucapkan:} "Ini hasil nb05 yang dijalankan di Colab pada tujuh September, dengan Tesla T4, mode penuh, dan warm-up dilakukan di kedua sisi. Baca syaratnya dulu, baru angkanya. Training CNN tiga epoch pada sepuluh ribu gambar CIFAR-10: seratus delapan belas detik di CPU, sembilan setengah detik di GPU, sekitar dua belas kali. Perkalian matriks delapan ribu kali delapan ribu: hampir sepuluh detik lawan setengah detik, sekitar sembilan belas kali. Sekarang yang menarik: inferensi seribu gambar hanya satu koma tiga tiga detik lawan satu koma satu empat detik. Hampir sama. Batch inferensinya kecil, jadi biaya memindahkan data dan memanggil GPU sebanding dengan hitungannya. Lalu mixed precision. Di model kecil ini justru lebih lambat: empat belas detik lawan sembilan setengah detik float32. Itu bukan bug. Tensor Core baru terasa pada perkalian matriks yang besar; pada model sekecil ini, konversi presisinya yang mendominasi. Pesannya: ukur, jangan asumsi. Terakhir, engine TensorRT FP16 melayani inferensi dalam empat belas setengah milidetik, sedangkan TensorFlow FP32 seratus delapan belas milidetik. Delapan kali, tapi sebut apa adanya: dua hal berubah sekaligus, engine dan presisinya. Kalau kamu menjalankan ulang di sesi lain, angkanya akan berbeda sedikit; syaratnya yang harus tetap dicatat. Tanya ke kelas: mana dari lima baris ini yang paling mengejutkan, dan kenapa?"\\[2pt]

\subsection*{\framebox{Slide 57/74}\quad Kapan GPU sepadan, dan apa jebakannya}
\addcontentsline{toc}{subsection}{Slide 57: Kapan GPU sepadan}
\textbf{Ucapkan:} "Kolom kiri, keadaan yang sepadan: datanya gambar atau urutan panjang dan modelnya berlapis; batchnya cukup besar dan epochnya banyak; banyak percobaan, artinya arsitekturnya diganti-ganti dan trainingnya diulang; atau inferensinya melayani banyak permintaan sekaligus. Kolom kanan, keadaan yang kurang sepadan: data tabular kecil seperti hari satu; satu prediksi sesekali di server biasa; hitungannya ringan sehingga waktunya habis untuk memindahkan data; dan yang terakhir ini paling sering terjadi, modelnya belum benar sehingga masalahnya bukan kecepatan. Tegaskan sekali: GPU mempercepat hitungan, bukan memperbaiki model. Kalau lossmu tidak turun, yang perlu diperiksa data, learning rate, dan arsitekturnya. GPU hanya membuat percobaan yang sama selesai lebih cepat, termasuk percobaan yang salah."\\[2pt]

\subsection*{\framebox{Slide 58/74}\quad Ringkasan Act 5}
...
\subsection*{\framebox{Slide 59/74}\quad Act 6: Bagaimana model menghasilkan data baru?}
...
\subsection*{\framebox{Slide 62/74}\quad Latent space: alamat 32 angka untuk tiap gambar}
...
\subsection*{\framebox{Slide 63/74}\quad Intuisi: GAN: pemalsu lukisan melawan kurator galeri}
\addcontentsline{toc}{subsection}{Slide 63: Intuisi: GAN}
\textbf{Ucapkan:} "Dua pemain, dan keduanya berlatih dari pertandingan yang sama. Pemain pertama pemalsu lukisan: dari noise acak, dia membuat gambar. Pemain kedua kurator galeri: dia diberi campuran gambar dari dataset dan gambar buatan pemain pertama, dan tugasnya membedakan keduanya. Sampai di situ analoginya; sekarang pakai nama sebenarnya, generator dan discriminator. Panah balik ini menunjukkan bagaimana loss dipakai untuk menghitung gradient yang memperbarui kedua model. Discriminator menghasilkan skor untuk gambar asli dan gambar buatan generator. Skor itu dipakai untuk menghitung dua loss dengan tujuan berbeda; di nb06 keduanya binary cross-entropy dari skor, bukan hitungan berapa gambar yang salah diklasifikasikan. Discriminator dilatih agar membedakan keduanya. Generator dilatih agar gambar buatannya mendapat skor seperti gambar asli. Kedua model diperbarui dalam proses training yang sama --- dua loss, dua perhitungan gradient, dua optimizer --- tetapi prosesnya tidak selalu stabil, dan tidak ada jaminan keduanya terus membaik. Satu hal yang praktis: untuk menghasilkan gambar setelah training, kita memakai generator."\\[2pt]

\subsection*{\framebox{Slide 64/74}\quad Digit yang lahir dari noise, epoch demi epoch}
\addcontentsline{toc}{subsection}{Slide 64: Hasil GAN}
\textbf{Ucapkan:} "Hasil nyata: DCGAN pada MNIST, 15 epoch, seed 42. Cara membaca grid ini yang penting. Tiap kolom memakai noise masukan yang sama, jadi dari baris atas ke bawah yang berubah hanya generatornya. Baris epoch satu masih berbintik, hampir tidak berbentuk. Baris epoch lima mulai punya goresan. Baris epoch 15 sebagian sudah berbentuk digit yang bisa dikenali, dan gambar-gambar itu bukan salinan dari dataset; semuanya keluar dari noise. Sekarang kegagalan yang paling khas dari GAN: mode collapse. Generator bisa menemukan satu atau dua bentuk yang lolos terus dari discriminator, lalu berhenti belajar variasi lain. Gejalanya keluaran yang mirip satu sama lain, jadi yang diperiksa keragamannya, bukan hanya kerapiannya. Satu grid yang isinya sepuluh angka delapan yang bagus adalah GAN yang gagal."\\[2pt]

\subsection*{\framebox{Slide 65/74}\quad Intuisi: LSTM yang sama, tugas yang digeser}
\addcontentsline{toc}{subsection}{Slide 65: Intuisi: LSTM menulis teks}
\textbf{Ucapkan:} "Dua bagian tadi menghasilkan gambar. Sekarang teks, dan modelnya bukan model baru: LSTM yang sama dengan yang kita pakai di nb04. Di sana LSTM membaca urutan lalu memprediksi satu hal di ujungnya, nilai sinus berikutnya atau sentimen sebuah ulasan. Di sini tugasnya digeser sedikit saja, dan gambarnya menunjukkan seluruh idenya. Masukan kiri, potongan teks: b-a-h-a-s-a spasi I-n-d-o-n-e-s-i. LSTM di tengah. Keluaran kanan, satu huruf: a. Lalu panah oranye yang membalik ke bawah: huruf itu disambung ke ujung masukan, dan modelnya diminta memprediksi lagi. Diulang tiga ratus kali, dan terbentuklah paragraf. Satu perbedaan penting dari autoencoder dan GAN dua slide tadi: di sini tidak ada noise sebagai bahan baku. Masukannya teks biasa. Yang menghasilkan variasi adalah cara kita mengambil sampel dari keluaran model, dan itu slide berikutnya. Di nb06 setelan lengkapnya begini: korpus Wikipedia Indonesia 320 ribu karakter, kamusnya 74 karakter berbeda, jendela 100 karakter, dan modelnya Embedding, satu LSTM 256, satu Dense --- 351 ribu parameter, 15 epoch, sekitar satu menit di T4. Kecil sekali untuk ukuran model bahasa, dan itu memang maksudnya: mekanismenya utuh, ukurannya yang berbeda."\\[2pt]

\subsection*{\framebox{Slide 66/74}\quad Temperature: satu angka yang mengatur keberanian}
\addcontentsline{toc}{subsection}{Slide 66: Temperature dan jebakan teks}
\textbf{Ucapkan:} "Keluaran model bukan satu huruf, melainkan skor untuk setiap huruf di kamus. Kalau kita selalu mengambil skor tertinggi, teksnya cepat terjebak mengulang pola yang sama. Karena itu huruf berikutnya diambil secara acak, mengikuti probabilitas dari skor tadi. Temperature adalah satu angka yang mengatur seberapa berani pengambilan acak itu. Turunkan ke nol koma tiga, dan bedanya menajam sampai model hampir selalu memilih huruf yang paling mungkin: hasilnya aman tetapi berulang; pada run referensi keluarannya berputar di 'dan' lalu 'dan' lagi. Naikkan ke satu koma dua, dan bedanya merata sampai huruf yang jarang pun berpeluang terpilih: hasilnya beragam tetapi banyak kata yang tidak ada dalam bahasa Indonesia. Yang di tengah biasanya paling enak dibaca. Sebut sekarang bahwa parameter dengan nama yang sama muncul di API model bahasa yang kita pakai di Modul 4, jadi ini bukan istilah lokal notebook ini. Sekarang kotak jebakannya, dan ini pesan yang paling ingin saya titipkan dari bagian teks. Kalimatnya terdengar Indonesia, tetapi tidak bermakna. Pada run referensi, setelah epoch satu keluarannya masih bunyi Indonesia tanpa kata yang benar-benar ada --- 'besunga mang puri mekeluran'. Setelah epoch 15 kata Indonesia nyata mulai muncul dengan tata bahasa yang masih rusak --- 'bahasa Indonesia yang berapi dan pertanah dan pengerang'. Itu run referensi di CPU, loss dua koma delapan dua turun ke satu koma empat tiga; angkamu bisa berbeda. Kenapa berhenti di situ: model ini hanya melihat 100 karakter ke belakang dan hanya belajar dari korpus 320 ribu karakter, jadi yang ditangkapnya ejaan, akhiran, dan irama kalimat, bukan isi."\\[2pt]
\textbf{Baca rumus:} $p = \mathrm{softmax}(\text{logit} / T)$ dibaca ``p sama dengan softmax dari logit dibagi T''. Tambahkan penjelasan lisan: ``kalau T kecil, angka yang dibagi jadi besar, jarak antar skor melebar, dan probabilitas menumpuk di satu huruf; kalau T besar, jaraknya menyempit dan probabilitasnya lebih merata''.\\[2pt]

\subsection*{\framebox{Slide 67/74}\quad Diffusion: noise ditambahkan bertahap, lalu dibalik}
\addcontentsline{toc}{subsection}{Slide 67: Diffusion}
...
\subsection*{\framebox{Slide 68/74}\quad Peta keluarga generatif}
\addcontentsline{toc}{subsection}{Slide 68: Peta keluarga generatif}
\textbf{Ucapkan:} "Satu tabel untuk merapikan enam nama yang beredar. Autoencoder: encoder membentuk representasi, decoder merekonstruksi input; dipakai untuk kompresi dan denoising. Sebut batasnya sekali: decoder autoencoder biasa tidak otomatis menjadi pembangkit yang baik dari titik acak. VAE: mempelajari representasi probabilistik, lalu sampel dari distribusi latent yang ditentukan didekode menjadi data --- dan itu jawaban atas batas yang kita lihat di Slide 62. GAN: generator dan discriminator dilatih dengan tujuan yang saling berlawanan, contohnya StyleGAN. Diffusion: belajar mengurangi noise pada berbagai tingkat, dan generasinya dilakukan bertahap; contohnya Stable Diffusion. Dua baris terakhir yang paling perlu dibaca berpasangan. RNN/LSTM autoregresif: memprediksi unit berikutnya dari unit sebelumnya, dan contohnya bukan model orang lain --- itu LSTM karakter yang baru saja kamu latih sendiri di nb06. Transformer autoregresif: memprediksi token berikutnya berdasarkan token sebelumnya. Perhatikan katanya: Transformer adalah arsitektur yang dipakai untuk banyak tugas, dan yang kita bicarakan di sini pemakaiannya secara autoregresif. Mekanismenya sama persis dengan baris di atasnya; yang berbeda arsitekturnya, unitnya token bukan karakter, konteksnya jauh lebih panjang, dan data latihnya jauh lebih besar. Baris paling bawah itulah yang kita pakai di Modul 4. Pada model bahasa yang akan kita bahas, keluarannya berupa token teks. Idenya tetap sama, menghasilkan sesuatu yang wajar menurut data yang pernah dilihat."\\[2pt]

\subsection*{\framebox{Slide 69/74}\quad Kapan model generatif cocok, dan apa jebakannya}
\addcontentsline{toc}{subsection}{Slide 69: Kapan model generatif cocok}
\textbf{Ucapkan:} "Kolom kiri, keadaan yang cocok: butuh contoh baru, misalnya menambah variasi data atau mengisi bagian yang hilang; butuh ringkasan padat, jadi kompresi, denoising, dan deteksi anomali dari galat rekonstruksi; atau hasilnya dinilai orang, bukan dicocokkan dengan satu jawaban. Kolom kanan, keadaan yang kurang cocok: pertanyaannya klasifikasi atau prediksi satu angka; jawabannya harus bisa diperiksa benar atau salah; atau data trainnya sedikit dan sempit, sehingga yang dihasilkan hanya mengulang yang itu-itu saja. Ini yang paling penting dibawa ke Modul 4. Model generatif menghasilkan yang wajar menurut data trainnya, belum tentu yang benar, dan akurasi tidak bisa dipakai menilainya. Di Modul 5 kita lihat satu cara mengikat keluaran ke sumber yang bisa ditunjuk, supaya klaimnya bisa diperiksa."\\[2pt]

\subsection*{\framebox{Slide 70/74}\quad Ringkasan Act 6}
\addcontentsline{toc}{subsection}{Slide 70: Ringkasan Act 6}
\textbf{Ucapkan:} "Enam hal dari blok generatif. Satu, model diskriminatif memetakan data ke label, sementara model generatif belajar seperti apa data yang wajar lalu menghasilkan contoh baru. Dua, autoencoder memampatkan gambar menjadi beberapa angka dan membangunnya ulang, dan decodernya bisa dipakai sendiri sebagai generator. Tiga, latent space menyimpan struktur, terlihat dari interpolasi yang mulus, sementara titik acak di daerah kosong masih memberi hasil kabur. Empat, GAN melatih generator dan discriminator sebagai lawan tanding; hasilnya bisa bagus, tetapi rewel dan rentan mode collapse. Lima, diffusion membalik penambahan noise selangkah demi selangkah: mahal saat training, dan bertahap saat menghasilkan gambar. Enam, untuk teks, LSTM memprediksi karakter berikutnya lalu menyambungnya, dan temperature mengatur keberaniannya; mekanisme itu sudah kamu jalankan sendiri di nb06, dan dipakai lagi di Modul 4, hanya dengan unit token dan arsitektur transformer."\\[2pt]

\section{Penutup}

\subsection*{\framebox{Slide 71/74}\quad Enam notebook hari ini}
\addcontentsline{toc}{subsection}{Slide 71: Enam notebook}
...
\subsection*{\framebox{Slide 72/74}\quad Apa yang menyusul sesudah hari 2}
\addcontentsline{toc}{subsection}{Slide 72: Apa selanjutnya}
\textbf{Ucapkan:} "Empat modul di depan, dan yang perlu ditekankan bekal hari ini dipakai terus. Modul 3, NLP: teks menjadi angka, token dan vektor kata, dan layer Embedding tadi pintu masuknya. Modul 4, LLM: transformer dan model bahasa. Modul 5, RAG: menjawab dengan sumber yang bisa ditunjuk, dan itu jawaban atas jebakan di Slide 69. Modul 6, ekosistem NVIDIA: optimasi, deploy, dan sisi tanggung jawab, dan jalur ONNX serta TensorRT hari ini muncul lagi di sana, kali ini untuk model bahasa. Tiga butir di bawah menyebut yang dibawa: loss, gradient descent, overfitting, embedding, dan cara memakai GPU. Model urutan hari ini juga pintu masuk ke transformer, jadi kalau Act 4 terasa berat, bagian itu akan terbayar besok."\\[2pt]

\subsection*{\framebox{Slide 73/74}\quad Latihan mandiri sebelum hari 3}
\addcontentsline{toc}{subsection}{Slide 73: Latihan mandiri}
...
\subsection*{\framebox{Slide 74/74}\quad Terima kasih}
\addcontentsline{toc}{subsection}{Slide 74: Terima kasih}
