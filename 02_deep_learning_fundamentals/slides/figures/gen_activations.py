...
turunannya (2 baris x 4 kolom), dengan daerah gradien ~0 ditandai (saturasi
sigmoid/tanh, dan sisi negatif ReLU).
...
for col, name in enumerate(names):
    ...
    ax_fn.set_title(name, color=TEXT, fontsize=11, fontweight="bold")
    ...
    ax_d.set_title(f"turunan {name}", color=TEXT, fontsize=10)
    ax_d.axhline(0, color="#888899", linewidth=0.8)
    ax_d.set_xlabel("x", color=TEXT, fontsize=9)
...
axes[0, 0].set_ylabel("f(x)", color=TEXT, fontsize=10)
axes[1, 0].set_ylabel("f'(x)", color=TEXT, fontsize=10)
...
fig.legend(handles, ["daerah gradien ~0 (saturasi / mati)"],
           loc="lower center", ncol=1, framealpha=0.3,
           facecolor="#2A2A4E", edgecolor="#555577",
           labelcolor=TEXT, fontsize=10, bbox_to_anchor=(0.5, -0.02))

fig.suptitle("Fungsi Aktivasi dan Turunannya", color=TEXT, fontsize=16,
