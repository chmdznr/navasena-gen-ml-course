...
ax.set_title("Skor mentah (logit)", color=TEXT, fontsize=13, fontweight="bold")
ax.set_ylabel("skor", color=TEXT, fontsize=11)
...
for rect, v in zip(bars2, probs):
    ax2.text(rect.get_x() + rect.get_width() / 2, v + 0.02, f"{v:.2f}",
             ha="center", va="bottom", color=TEXT, fontsize=11, fontweight="bold")
ax2.set_title("Setelah softmax (probabilitas, total = 1.00)", color=TEXT,
              fontsize=13, fontweight="bold")
ax2.set_ylabel("probabilitas", color=TEXT, fontsize=11)
ax2.set_ylim(0, 1.05)

for ax_ in axes:
    ax_.tick_params(colors=TEXT, labelsize=10)
    for spine in ax_.spines.values():
        spine.set_edgecolor("#444466")
    ax_.grid(True, axis="y", color=GRID, linestyle="--", linewidth=0.6, zorder=0)
    ax_.set_axisbelow(True)

fig.suptitle("Softmax: dari skor mentah ke probabilitas", color=TEXT,
