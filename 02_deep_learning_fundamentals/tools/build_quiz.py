#!/usr/bin/env python3
"""Build the Module 02 DL Fundamentals quiz HTML — 30 concept questions (code=null).
...
Q = [
    ...
    ("Loss training turun terus sampai epoch 30, tetapi loss validation mulai naik sejak epoch 8. Apa yang terjadi dan apa langkah yang wajar?",
     ["Underfitting; tambah layer dan lanjutkan sampai epoch 60 supaya loss validation ikut turun",
      "Overfitting sejak epoch 8; pakai early stopping atau dropout, ambil bobot epoch terbaik",
      "Learning rate terlalu kecil; naikkan sepuluh kali lipat agar keduanya turun bersama",
      "Data validation rusak; ganti dengan data test supaya kurvanya lebih halus"], 1,
     "Gap yang membesar antara train dan validation adalah tanda overfitting. Early stopping/dropout menahannya; test set tidak boleh dipakai untuk memilih epoch."),
    ("Saat gradient descent, loss melonjak naik-turun dan akhirnya menjadi NaN. Penyebab paling mungkin?",
     ["Batch size terlalu kecil sehingga gradien menjadi terlalu halus dan tidak bergerak",
    ...
    ("Mengapa piksel gambar dibagi 255 sebelum masuk ke neural network?",
    ...
    ("Apa yang sebenarnya dilakukan backpropagation?",
     ["Menjalankan data dari belakang ke depan supaya model membaca gambar secara terbalik",
      "Menghitung andil tiap bobot pada error, diteruskan dari output ke layer awal",
     ...
     "Backprop mendistribusikan error keluaran ke setiap bobot lewat turunan berantai; gradient descent lalu memperbarui bobotnya."),
    ...
    ("Dropout 0,5 dipasang di layer tersembunyi. Apa yang terjadi saat training dan saat prediksi?",
     ["Saat training separuh neuron dimatikan acak; saat prediksi semua aktif",
     ...
     "Dropout hanya aktif saat training: neuron dimatikan acak agar model tidak bergantung pada jalur tertentu. Saat inferensi semua neuron dipakai."),
    # --- Aktivasi & optimizer (4) ---
    ("Kamu menumpuk lima layer Dense tanpa fungsi aktivasi. Apa kemampuan model ini?",
     ["Setara satu layer linear: tumpukan transformasi linear tetap linear",
      "Lima kali lebih kuat dari satu layer karena setiap layer menambah pola baru yang bisa ditangkap",
    ...
    ("Network 20 layer dengan aktivasi sigmoid di semua layer belajar sangat lambat; bobot layer awal hampir tidak berubah. Apa penyebabnya?",
     ["Vanishing gradient: turunan sigmoid ≤ 0,25, dikalikan berulang jadi hampir nol",
      "Exploding gradient: sigmoid memperbesar gradien setiap layer sehingga bobot awal menjadi tak terhingga",
     ...
     "Turunan sigmoid kecil; perkalian berantai lewat banyak layer membuat gradien di layer awal menyusut. ReLU mengurangi masalah ini."),
    ("Layer keluaran untuk klasifikasi 10 kelas memakai softmax. Apa yang dijamin softmax pada keluarannya?",
     ["Sepuluh nilai 0–1 yang berjumlah tepat 1, bisa dibaca sebagai probabilitas kelas",
      "Sepuluh nilai yang masing-masing bisa negatif atau positif, mencerminkan skor mentah tiap kelas",
    ...
    ("Apa perbedaan praktis Adam dibanding SGD biasa?",
     ["Adam menyesuaikan besar langkah tiap parameter dari riwayat gradien, konvergen lebih cepat",
      "Adam memakai learning rate yang jauh lebih besar sehingga selalu mencapai loss lebih rendah di akhir",
      "Adam mengubah arsitektur model dengan menambah neuron secara otomatis saat loss stagnan",
      "Adam hanya berfungsi untuk gambar, sedangkan SGD untuk data tabular dan teks"], 0,
    ...
    ("Mengapa CNN memakai filter kecil (misalnya 3×3) yang digeser ke seluruh gambar, bukan Dense yang terhubung ke semua piksel?",
     ["Filter yang sama dipakai di semua posisi: parameter sedikit, pola lokal terdeteksi di mana pun",
      "Karena gambar tidak bisa diratakan menjadi vektor, sehingga Dense secara teknis tidak bisa menerima piksel",
      "Karena filter 3×3 melihat seluruh gambar sekaligus sehingga konteks globalnya lebih lengkap dari Dense",
      "Karena Dense hanya bekerja pada gambar hitam-putih, sedangkan konvolusi mendukung warna"], 0,
     "Konvolusi berbagi bobot antar posisi (translation invariance) dan hanya melihat tetangga lokal; itu hemat parameter dan cocok untuk gambar."),
    ("Apa yang ditunjukkan sebuah feature map di layer konvolusi pertama?",
     ["Peta respons satu filter di tiap posisi gambar, misalnya tepi vertikal atau horizontal",
      ...
      "Salinan gambar asli yang sudah dinormalisasi dan diperkecil ke ukuran 3×3 piksel",
    ...
    ("Apa fungsi pooling (misalnya max pooling 2×2) di CNN?",
     ["Mengecilkan feature map dan membuat deteksi tahan pergeseran kecil, ambil nilai terbesar tiap blok",
      ...
      "Menormalkan nilai piksel ke rentang 0–1 sebelum masuk ke layer konvolusi berikutnya"], 0,
     "Pooling merangkum blok kecil menjadi satu nilai; ukuran turun, komputasi hemat, dan posisi persis fitur menjadi kurang penting."),
    ("Kamu punya 5.000 foto produk dan ingin classifier gambar yang bagus dalam satu sore. Pendekatan mana yang paling masuk akal?",
     ["Transfer learning: backbone pre-trained ImageNet dibekukan, latih layer atas dengan datamu",
      ...
      "Pakai Dense besar tanpa konvolusi supaya semua piksel diperhitungkan tanpa kehilangan detail",
      "Kumpulkan dulu satu juta foto tambahan, karena CNN tidak bisa belajar dari data sebanyak itu"], 0,
     "Dengan data terbatas, fitur umum dari ImageNet (tepi, tekstur, bentuk) bisa dipakai ulang; hanya kepala classifier yang dilatih."),
    ("ResNet50 pre-trained ImageNet dipakai untuk CIFAR-10 berukuran 32×32 dan hasilnya tidak sebaik yang diharapkan. Penjelasan mana yang paling masuk akal?",
     ["Fitur ImageNet dipelajari dari gambar ±224×224; di 32×32 detail yang dikenali backbone hampir tak ada",
      "ResNet50 hanya bisa mengenali 1.000 kelas ImageNet dan menolak dataset dengan 10 kelas",
      "Transfer learning selalu lebih buruk dari CNN dari nol pada dataset yang lebih kecil dari ImageNet",
      "CIFAR-10 adalah gambar hitam-putih sehingga tiga channel warna ResNet tidak terpakai"], 0,
     "Kecocokan resolusi dan domain menentukan manfaat transfer learning; gambar sangat kecil membuat fitur pre-trained kurang relevan."),
    ("Augmentasi seperti RandomFlip dan RandomRotation dipasang sebagai layer di dalam model. Apa efeknya saat training dan saat prediksi?",
     ["Saat training tiap batch melihat variasi acak; saat prediksi layer ini tidak mengubah gambar",
      "Saat training gambar tetap asli; saat prediksi gambar diputar acak supaya model diuji lebih keras",
      "Jumlah gambar di disk bertambah dua kali lipat karena setiap gambar disimpan versi terbaliknya",
      "Model menjadi lebih kecil karena gambar yang mirip digabung menjadi satu contoh"], 0,
    ...
    ("Apa yang dibawa 'hidden state' pada RNN dari satu langkah waktu ke langkah berikutnya?",
     ["Ringkasan masukan sebelumnya, sehingga prediksi kini bergantung pada konteks yang lewat",
      "Salinan persis seluruh masukan sebelumnya, sehingga memori yang dibutuhkan tumbuh seiring panjang urutan",
    ...
    ("SimpleRNN gagal menangkap hubungan antara kata pertama dan kata ke-80 dalam sebuah ulasan. Mengapa, dan apa yang membantu?",
     ...
     "Di urutan panjang gradien RNN biasa menghilang; gerbang LSTM/GRU mengatur apa yang disimpan dan dilupakan sehingga konteks jauh bisa bertahan."),
    ("Apa peran layer Embedding pada model teks?",
     ["Mengubah indeks kata menjadi vektor padat yang dipelajari; kata bermakna serupa berdekatan",
      "Menghapus kata-kata umum seperti 'dan' dan 'yang' supaya model fokus pada kata penting",
      "Menerjemahkan ulasan ke bahasa Inggris sebelum diproses oleh LSTM",
      "Mengurutkan kata berdasarkan frekuensi lalu membuang yang jarang muncul"], 0,
    ...
    ("Untuk memprediksi nilai deret waktu berikutnya, baseline paling sederhana yang wajar dibandingkan dengan RNN adalah?",
     ["Memprediksi bahwa nilai berikutnya sama dengan nilai terakhir yang diketahui",
      "Memprediksi nol untuk semua langkah supaya error mudah dihitung",
      "Memakai rata-rata seluruh deret termasuk data masa depan yang belum terjadi",
      "Menjalankan LSTM dengan satu neuron dan menyebutnya baseline"], 0,
    ...
    ("Mengapa training deep learning cocok untuk GPU?",
     ["Kerjanya didominasi perkalian matriks besar yang bisa dipecah jadi ribuan operasi paralel",
      ...
      "GPU menghitung gradien secara simbolik sehingga backprop tidak perlu dilakukan"], 0,
    ...
    ("Model kecil (dua layer Dense, batch 32, data 1.000 baris) dilatih di GPU ternyata tidak lebih cepat dari CPU. Penjelasan yang tepat?",
     ["Kerja per langkah terlalu kecil; overhead pindah data dan kernel GPU melebihi hitungannya",
      "GPU Colab sedang rusak, karena GPU seharusnya selalu lebih cepat untuk model apa pun",
      "TensorFlow tidak mendukung GPU untuk layer Dense, hanya untuk konvolusi",
      "Batch 32 terlalu besar untuk GPU sehingga harus dipecah menjadi batch 1"], 0,
    ...
    ("Mixed precision (FP16 untuk sebagian komputasi) dipakai saat training. Apa manfaat dan syaratnya?",
     ["Lebih cepat dan hemat memori di GPU ber-FP16; layer keluaran tetap float32 demi stabilitas",
      "Model menjadi dua kali lebih akurat karena angka dihitung dengan dua presisi sekaligus",
      "Semua bobot menjadi integer sehingga model bisa dijalankan di mikrokontroler",
      "Training menjadi deterministik sehingga hasil selalu identik di setiap run"], 0,
     "FP16 mempercepat matmul di Tensor Core dan mengecilkan memori; keluaran dan loss dijaga float32 demi stabilitas numerik."),
    ...
    ("Apa perbedaan mendasar model diskriminatif dan generatif?",
     ["Diskriminatif memetakan masukan ke label; generatif mempelajari sebaran data dan membuat contoh baru",
      "Diskriminatif bekerja pada gambar, generatif hanya pada teks dan suara",
      "Diskriminatif dilatih tanpa label, generatif membutuhkan label untuk setiap contoh",
      "Diskriminatif selalu lebih besar dan lebih lambat daripada model generatif"], 0,
     "Classifier menjawab 'kelas apa'; model generatif memodelkan bagaimana data terbentuk sehingga bisa membuat sampel baru."),
    ("Autoencoder dilatih merekonstruksi digit MNIST lewat bottleneck 2 dimensi. Apa arti 'latent space' di sini?",
     ["Ruang 2 dimensi ringkasan tiap gambar; titik berdekatan didekode menjadi digit yang mirip",
      "Daftar 784 piksel asli yang disimpan ulang tanpa perubahan di tengah model",
      "Label 0–9 yang disisipkan ke model supaya rekonstruksinya lebih akurat",
      "Learning rate dan batch size yang dipelajari otomatis oleh encoder"], 0,
    ...
    ("Dalam GAN, generator dan discriminator dilatih bersama. Apa peran masing-masing?",
     ["Generator membuat gambar palsu dari noise; discriminator menebak asli/palsu dan menjadi sinyal belajar",
      "Generator memberi label pada gambar; discriminator menghapus gambar yang labelnya salah",
      "Generator dan discriminator adalah dua salinan model yang sama, dilatih pada data berbeda lalu dirata-rata",
    ...
    ("Setelah 15 epoch, DCGAN menghasilkan digit yang semuanya terlihat seperti angka 1. Fenomena ini disebut apa?",
     ["Mode collapse: generator menemukan satu keluaran yang menipu discriminator dan berhenti bervariasi",
      ...
      "Vanishing gradient pada decoder: gradien tidak sampai ke layer awal generator sehingga keluarannya seragam",
...
<footer>Navasena Gen-ML Course · Module 01 Deep Learning Fundamentals — Quiz</footer>
</div>
<script>
const QUIZ = __JSON__;
window.QUIZ = QUIZ;
