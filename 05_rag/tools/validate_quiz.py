#!/usr/bin/env python3
"""Gate for the reworked Module 05 RAG quiz. Parses the inline QUIZ JSON and checks invariants.
Mirrors 04_llm/tools/validate_quiz.py."""
import re, json, sys, pathlib

ROOT = pathlib.Path(__file__).resolve().parent.parent
HTML = (ROOT / "rag-fundamentals-quiz.html").read_text(encoding="utf-8")
errs = []
...
for kw in ["rerank", "ragas", "faithfulness", "chunk", "sitasi", "fastapi", "embedding", "rewriting", "halusinasi"]:
    if kw not in blob:
        errs.append(f"coverage gap: '{kw}' not found in any question")

if errs:
    print("QUIZ VALIDATION FAILED:"); [print("  -", e) for e in errs]; sys.exit(1)
print(f"QUIZ OK: {n} pure-concept questions, header matches, 8-topic RAG coverage.")
