#!/usr/bin/env python3
"""Gate quiz Modul 03 NLP. Membaca payload QUIZ inline lalu memeriksa invariannya."""
import re, json, sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
from bahasa_rules import check

ROOT = pathlib.Path(__file__).resolve().parent.parent
HTML = (ROOT / "nlp-fundamentals-quiz.html").read_text(encoding="utf-8")
errs = []
...
for stale in ["spam vs ham", "notebook bahasa indonesia", "_id.ipynb", "_en.ipynb", "sastrawi adalah pilihan utama"]:
    if stale in blob:
        errs.append(f"stale stub content present: {stale!r}")

# Coverage: the 8-notebook concept arc must appear across the questions.
for kw in ["stopword", "lemmatisasi", "stemming", "pos tagging", "ner", "word cloud", "leksikon",
           "indobert", "bag-of-words", "naive bayes", "kebocoran data", "baseline",
           "tf-idf", "idf", "embedding", "cosine", "semantic search", "faiss", "subword",
           "token", "cudf", "cuml", "warm-up", "minhash", "nvtext"]:
    if kw not in blob:
...
print(f"QUIZ OK: {n} soal konsep murni, header cocok, cakupan konsep M03 lengkap.")
