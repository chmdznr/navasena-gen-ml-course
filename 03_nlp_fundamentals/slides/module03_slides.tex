...
\begin{frame}{Batas Leksikon: Negasi}
  \begin{center}
  \resizebox{0.70\linewidth}{!}{%
  \begin{tikzpicture}[font=\small]
    \node[text=nvwhite, font=\normalsize] at (0,2.2) {"the material is \textbf{not bad} at all"};
    \node[draw=nvcard, fill=nvdarkcard, rounded corners=4pt, text=nvwhite,
          minimum width=2.2cm, minimum height=0.7cm] (w1) at (-3.2,0.6) {not};
    \node[draw=nvred, fill=nvcard, rounded corners=4pt, text=nvred,
          minimum width=2.2cm, minimum height=0.7cm] (w2) at (-0.4,0.6) {bad};
    \node[draw=nvcard, fill=nvdarkcard, rounded corners=4pt, text=nvwhite,
          minimum width=2.2cm, minimum height=0.7cm] (w3) at (2.4,0.6) {at all};
    \node[text=nvgray, font=\footnotesize] at (0,-0.6) {leksikon menjumlahkan skor per kata};
    \node[text=nvorange, font=\small\bfseries] at (0,-1.3)
      {maksud manusia: pujian \quad $\neq$ \quad hasil penjumlahan};
  \end{tikzpicture}%
  }
  \end{center}
  \vspace{0.3em}
  \begin{itemize}
    \footnotesize
    \item Leksikon tidak membaca urutan, jadi negasi berlapis dan sarkasme mudah meleset
    \item Transformer membaca kalimat sebagai satu kesatuan --- di situlah bedanya
  \end{itemize}
\end{frame}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[font=\footnotesize]
        ...
        \node[text=nvwhite, align=left] at (0,0.4)
          {\texttt{outstanding}\quad pos : neg = 13,0 : 1,0\\[2pt]
           \texttt{insulting}\ \ \quad neg : pos = 11,0 : 1,0\\[2pt]
           \texttt{ludicrous}\ \ \quad neg : pos = \ 9,7 : 1,0};
      \end{tikzpicture}%
      }
      \end{center}
      \vspace{0.2em}
      {\footnotesize\color{nvgray}Dibaca: kata itu 13x lebih sering muncul di ulasan positif.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Baseline: Angka Pembanding yang Wajib}
  ...
  \begin{tikzpicture}[font=\small]
    \node[draw=nvcard, fill=nvcard, rounded corners=4pt, text=nvwhite,
          ...
          {\textbf{Akurasi model}\\[4pt]{\Large\color{nvgreen}82\%}};
    \node[draw=nvcard, fill=nvcard, rounded corners=4pt, text=nvwhite,
          text width=5.6cm, align=center, inner sep=8pt] (b) at (3.4,0)
          {\textbf{Baseline kelas mayoritas}\\[4pt]{\Large\color{nvorange}50\%}};
  \end{tikzpicture}%
  }
  \end{center}
  \vspace{0.8em}
  \begin{itemize}
    ...
    \item Di \texttt{movie\_reviews} kedua kelas seimbang, jadi baseline $\approx$ 50\%
...
\begin{frame}{TF-IDF Buta terhadap Sinonim}
  ...
  \begin{tikzpicture}[font=\small]
    ...
    \node[draw=nvgreen, fill=nvcard, rounded corners=4pt, text=nvgreen,
          ...
          {Embedding\\similarity $\approx$ 0,85};
  \end{tikzpicture}%
  }
  \end{center}
  \vspace{0.7em}
  ...
  \end{itemize}
  \vspace{0.2em}
  {\color{nvgray}\scriptsize Angka ilustratif; nilai persisnya dihitung di notebook 01 Section 9.}
\end{frame}
...
\begin{frame}
  ...
  \resizebox{0.85\linewidth}{!}{%
  \begin{tikzpicture}[font=\small]
    \node[text=nvwhite, font=\footnotesize] at (0,3.0) {kalimat yang maknanya sama, tokenizer GPT-2};
    \node[text=nvwhite, font=\footnotesize, left] at (-4.2,2.0) {Inggris};
    \fill[nvgreen] (-4.0,1.8) rectangle (0.2,2.2);
    \node[text=nvwhite, font=\footnotesize, right] at (0.4,2.0) {9 token};
    \node[text=nvwhite, font=\footnotesize, left] at (-4.2,1.0) {Indonesia};
    \fill[nvorange] (-4.0,0.8) rectangle (3.2,1.2);
    \node[text=nvwhite, font=\footnotesize, right] at (3.4,1.0) {17 token};
  \end{tikzpicture}%
  }
  \end{center}
  \vspace{0.5em}
  \begin{itemize}
    ...
    \item Tokenizer multibahasa atau khusus Indonesia biasanya menekan rasio ini
  \end{itemize}
  \vspace{0.2em}
  {\color{nvgray}\scriptsize Angka ilustratif; hitung sendiri di notebook 01 Section 10.}
\end{frame}
